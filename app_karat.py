...
# =========================
    ...
    with tab_month:
        section_header(
            "Месячная динамика переключений аудитории Violette по брендам",
            "База: покупатели Violette, которые также покупали другие бренды творожных сыров в соответствующем месяце; показаны top-10 брендов по суммарному числу покупателей в базе переключений за весь период."
        )
        ...
        if not f5_month.empty:
            ...
            if violette_brand is not None:
                st.caption(
                    "Цвет показывает число покупателей из базы переключений по бренду и месяцу. "
